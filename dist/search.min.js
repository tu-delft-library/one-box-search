...
`):""}
  ...
  >
    ...
    >,
    ${g.map((e)=>u`<a href="#${e.id}"><b>${r(e.title)}</b></a>`).join(", ")}.
  </div>`};async function b(e,a){y=a;let t=document.querySelector(".multiRowGrid");if(t){let n=document.querySelector("div.hiddenSidebarContent");if(n){let i=n.innerHTML.split("</form>")[1].trim(),o=k();n.innerHTML=n.innerHTML.split("</form>")[0]+o;let s=t.querySelector(".sm-12");if(s){s.id="website";let c=s.querySelector("h2");if(c){c.textContent=r({en:"This website",nl:"Deze website"});let p=document.createElement("p");p.style.fontStyle="italic",p.innerText=r({en:"Guidelines, tools, events, news",nl:"Richtlijnen, hulpmiddelen, evenementen en nieuwsberichten"}),c.after(p)}let l=s.querySelector(".btn");if(l)l.textContent=r({en:"View all results",nl:"Bekijk alle resultaten"});s.prepend(i?i:r({en:"No results",nl:"Geen resultaten"}))}}g.forEach(async(i,o)=>{let s=document.createElement("div");if(s.className="sm-6 md-6 lg-6",s.id=i.id,s.innerHTML=u`
        <div class="t3ce frame-type-gridelements_pi1">
          <div class="grid-background--white grid-background--boxed">
      ...
      `,o===0)t.prepend(s);else t.append(s);let c=await i.getRecords(e),l=C(i.title,i.description,c,c?.count,i.searchBaseUrl+e);s.innerHTML=l})}}window.onload=()=>{let{value:e}=document.querySelector(".searchForm-input"),a=document.documentElement.lang||"en";document.documentElement.style.setProperty("scroll-behavior","smooth"),b(e,a)};

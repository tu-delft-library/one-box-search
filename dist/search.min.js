...
`):""}
  ...
  `}async function f(i,e){g=e;let t=document.querySelector(".multiRowGrid");if(t){let n=document.querySelector("div.hiddenSidebarContent");if(n){let a=n.innerHTML.split("</form>")[1].trim();n.innerHTML=n.innerHTML.split("</form>")[0];let s=t.querySelector(".sm-12");if(s){let r=s.querySelector("h2");if(r)r.innerHTML=c({en:"This website",nl:"Deze website"});let o=s.querySelector(".btn");if(o)o.innerHTML=c({en:"View all results ↗",nl:"Bekijk alle resultaten ↗"});s.prepend(a?a:c({en:"No results",nl:"Geen resultaten"}))}}u.forEach(({id:a,title:s},r)=>{let o=document.createElement("div");if(o.className="sm-6 md-6 lg-6",o.id=a,o.innerHTML=p`
        <div class="t3ce frame-type-gridelements_pi1">
          <div class="grid-background--white grid-background--boxed">
      ...
      `,r===0)t.prepend(o);else t.appendChild(o)}),u.map(async(a)=>{let s=await a.getRecords(i),r=A(a.title,s,s?.count,a.searchBaseUrl+i),o=document.getElementById(a.id);if(o)o.innerHTML=r})}}window.onload=()=>{let{value:i}=document.querySelector(".searchForm-input"),e=document.documentElement.lang||"en";f(i,e)};

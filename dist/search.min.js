...
`):""}
  ...
  >
    ...
    >,
    ${Q.map(({id:f,title:E})=>K`<a href="#${f}"><b>${L(E)}</b></a>`).join(", ")}.
  </div>`};async function X(f,E){W=E;let m=document.querySelector(".multiRowGrid");if(m){let j=document.querySelector("div.hiddenSidebarContent");if(j){let $=j.innerHTML.split("</form>")[1].trim(),k=G();j.innerHTML=j.innerHTML.split("</form>")[0]+k;let F=m.querySelector(".sm-12");if(F){F.id="website";let M=F.querySelector("h2");if(M){M.textContent=L({en:"This website",nl:"Deze website"});let N=document.createElement("p");N.style.fontStyle="italic",N.innerText=L({en:"Guidelines, tools, events, news",nl:"Richtlijnen, hulpmiddelen, evenementen en nieuwsberichten"}),M.after(N)}let O=F.querySelector(".btn");if(O)O.textContent=L({en:"View all results",nl:"Bekijk alle resultaten"});F.prepend($?$:L({en:"No results",nl:"Geen resultaten"}))}}Q.forEach(async($,k)=>{let F=document.createElement("div");if(F.className="sm-6 md-6 lg-6",F.id=$.id,F.innerHTML=K`
        <div class="t3ce frame-type-gridelements_pi1">
          <div class="grid-background--white grid-background--boxed">
      ...
      `,k===0)m.prepend(F);else m.append(F);let M=await $.getRecords(f),O=g($.title,$.description,M,M?.count,$.searchBaseUrl+f);F.innerHTML=O})}}window.onload=()=>{let{value:f}=document.querySelector(".searchForm-input"),E=document.documentElement.lang||"en";document.documentElement.style.setProperty("scroll-behavior","smooth"),X(f,E)};
